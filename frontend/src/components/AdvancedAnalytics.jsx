...
const AdvancedAnalytics = ({
  ...
  const getAllTokensAggregated = () => {
    ...
    stakingData.forEach(position => {
      if (position.tokens && Array.isArray(position.tokens)) {
        const tokens = filterLendingDefiTokens ? filterLendingDefiTokens(position.tokens, showLendingDefiTokens) : position.tokens
        tokens.forEach(token => {
          const symbol = token.symbol
          const value = parseFloat(token.totalPrice) || 0
  ...
  const getProtocolDistribution = () => {
    ...
    stakingData.forEach(position => {
      const protocolName = position.protocol?.name || position.position?.protocol?.name || 'Staking'
    ...
    return Array.from(protocolMap.values())
      ...
      .map(protocol => ({
        ...protocol,
        percentage: ((protocol.value / (liquidityValue + lendingValue + stakingValue)) * 100).toFixed(1)
      }))
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{
            ...
            margin: 0
          }}>Deep insights into your portfolio composition</p>
        </div>
      </div>
      ...
      <div style={{
        display: 'grid',
        gridTemplateColumns: protocolDistribution.length > 0 
        ...
        <div style={{
          ...
          overflow: 'visible' // Garante que o gráfico não seja cortado
        }}>
          <h4 style={{
          ...
          <div style={{
            ...
            <div style={{ 
              ...
              {/* Usando CSS table para alinhamento perfeito */}
              <div style={{
                display: 'table',
                width: '100%',
                ...
                {distributionData.map((item, index) => (
                  <div key={index} style={{
                    ...
                    <div style={{
                      ...
                      verticalAlign: 'middle',
                      whiteSpace: 'nowrap'
                    }}>{maskValue(formatPrice(item.value))}</div>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div style={{
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{
                      ...fontStyles.normal,
                      fontWeight: 600
                    }}>{maskValue(formatPrice(token.value))}</div>
                    <div style={{
                      ...fontStyles.small
                    ...
                    <div key={index} style={{
                      ...
                      <div style={{
                        ...
                        <div style={{
                          ...
                          display: protocol.logo ? 'none' : 'block'
                        }} />
                      </div>
                      <div style={{
                        display: 'table-cell',
                      ...
                      <div style={{
                        ...
                        verticalAlign: 'middle',
                        whiteSpace: 'nowrap'
                      }}>{maskValue(formatPrice(protocol.value))}</div>
                    </div>
                  ))}
                </div>
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center', padding: 16 }}>
            <div style={{
              ...
              color: theme.textPrimary,
              marginBottom: 4
            }}>{liquidityData.length + lendingData.length + stakingData.length}</div>
            <div style={{
              ...fontStyles.small,
              textTransform: 'uppercase',
              letterSpacing: '0.5px'
